...
1. **引导流程**：全新数据首次启动出现引导窗口（说明 → 模板选择）；选模板后创建提醒并进入权限步骤；「继续」申请通知、「稍后设置」跳过；完成后不再次出现。
2. **权限延后**：未创建提醒前不弹系统权限框；已拒绝通知时引导内「继续」不重复弹窗，菜单栏/设置页显示修复入口。已有提醒的回归用户启动时会补一次授权申请。
3. **立即提醒一次**：菜单栏「下一个提醒」旁按钮点击后立即发出弱通知，列表状态变为「等待回应」。
